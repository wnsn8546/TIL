...
```sql 
SELECT is_drinking, waist, COUNT(*) FROM healthcare GROUP BY is_drinking;
``` 
```
...
```sql
SELECT is_drinking, smoking, weight / ((height * 0.01) * (height * 0.01)) AS BMI FROM healthcare WHERE smoking <> '' GROUP BY is_drinking, smoking;
```
```
